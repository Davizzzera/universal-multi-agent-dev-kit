...
type: skill
category: ai-agents
description: Design structured prompts with context, rules, output format and self-review.
when_to_use: When performing tasks related to prompt architecture.
do_not_use_when: When the task falls outside this domain or requires a different skill entirely.
risk_level: medium
owner_agents: [prompt-engineer, ai-engineer]
...
## Purpose
Design structured prompts with context, rules, output format and self-review.

## When to Use
Use this skill when tasked with executing a prompt architecture operation to ensure consistency and correctness.

## When Not to Use
Do not use this skill if the task does not involve prompt architecture, or if a more specific alternative skill exists.
...
## Procedure
1. Analyze the request and identify the specific requirements for prompt architecture.
2. Review existing patterns or codebase constraints before taking action.
3. Formulate a plan to execute the prompt architecture operation safely.
...
## Checklist
- [ ] Understand the exact goal of the prompt architecture request.
...
## Example Usage
*Scenario:* An agent is tasked with prompt architecture.
