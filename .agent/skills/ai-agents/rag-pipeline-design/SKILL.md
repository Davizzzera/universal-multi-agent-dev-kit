...
type: skill
category: ai-agents
description: Design retrieval-augmented generation flows with source grounding.
when_to_use: When performing tasks related to rag pipeline design.
do_not_use_when: When the task falls outside this domain or requires a different skill entirely.
risk_level: medium
owner_agents: [rag-specialist, ai-engineer]
...
## Purpose
Design retrieval-augmented generation flows with source grounding.

## When to Use
Use this skill when tasked with executing a rag pipeline design operation to ensure consistency and correctness.

## When Not to Use
Do not use this skill if the task does not involve rag pipeline design, or if a more specific alternative skill exists.
...
## Procedure
1. Analyze the request and identify the specific requirements for rag pipeline design.
2. Review existing patterns or codebase constraints before taking action.
3. Formulate a plan to execute the rag pipeline design operation safely.
...
## Checklist
- [ ] Understand the exact goal of the rag pipeline design request.
...
## Example Usage
*Scenario:* An agent is tasked with rag pipeline design.
