...
type: skill
category: ai-agents
description: Design multi-agent coordination patterns and dispatch logic.
when_to_use: When performing tasks related to multi agent orchestration.
...
## Purpose
Design multi-agent coordination patterns and dispatch logic.

## When to Use
Use this skill when tasked with executing a multi agent orchestration operation to ensure consistency and correctness.

## When Not to Use
Do not use this skill if the task does not involve multi agent orchestration, or if a more specific alternative skill exists.
...
## Procedure
1. Analyze the request and identify the specific requirements for multi agent orchestration.
2. Review existing patterns or codebase constraints before taking action.
3. Formulate a plan to execute the multi agent orchestration operation safely.
...
## Checklist
- [ ] Understand the exact goal of the multi agent orchestration request.
...
## Example Usage
*Scenario:* An agent is tasked with multi agent orchestration.
