---
name: subagent-definition
type: skill
category: ai-agents
description: Define focused subagents with isolated context and clear responsibilities.
when_to_use: When performing tasks related to subagent definition.
do_not_use_when: When the task falls outside this domain or requires a different skill entirely.
risk_level: medium
owner_agents: [agent-engineer, task-router]
...
## Purpose
Define focused subagents with isolated context and clear responsibilities.

## When to Use
Use this skill when tasked with executing a subagent definition operation to ensure consistency and correctness.

## When Not to Use
Do not use this skill if the task does not involve subagent definition, or if a more specific alternative skill exists.
...
## Procedure
1. Analyze the request and identify the specific requirements for subagent definition.
2. Review existing patterns or codebase constraints before taking action.
3. Formulate a plan to execute the subagent definition operation safely.
...
## Checklist
- [ ] Understand the exact goal of the subagent definition request.
...
## Example Usage
*Scenario:* An agent is tasked with subagent definition.
