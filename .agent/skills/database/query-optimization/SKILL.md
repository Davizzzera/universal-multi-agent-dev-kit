...
type: skill
category: database
description: Improve slow or complex queries without changing intended results.
when_to_use: When performing tasks related to query optimization.
do_not_use_when: When the task falls outside this domain or requires a different skill entirely.
risk_level: medium
owner_agents: [sql-specialist, performance-engineer]
...
## Purpose
Improve slow or complex queries without changing intended results.

## When to Use
Use this skill when tasked with executing a query optimization operation to ensure consistency and correctness.

## When Not to Use
Do not use this skill if the task does not involve query optimization, or if a more specific alternative skill exists.
...
## Procedure
1. Analyze the request and identify the specific requirements for query optimization.
2. Review existing patterns or codebase constraints before taking action.
3. Formulate a plan to execute the query optimization operation safely.
...
## Checklist
- [ ] Understand the exact goal of the query optimization request.
...
## Example Usage
*Scenario:* An agent is tasked with query optimization.
