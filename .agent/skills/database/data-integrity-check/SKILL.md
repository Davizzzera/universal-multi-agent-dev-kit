...
type: skill
category: database
description: Validate data consistency, constraints and referential integrity.
when_to_use: When performing tasks related to data integrity check.
do_not_use_when: When the task falls outside this domain or requires a different skill entirely.
risk_level: medium
owner_agents: [database-specialist, data-quality-specialist]
...
## Purpose
Validate data consistency, constraints and referential integrity.

## When to Use
Use this skill when tasked with executing a data integrity check operation to ensure consistency and correctness.

## When Not to Use
Do not use this skill if the task does not involve data integrity check, or if a more specific alternative skill exists.
...
## Procedure
1. Analyze the request and identify the specific requirements for data integrity check.
2. Review existing patterns or codebase constraints before taking action.
3. Formulate a plan to execute the data integrity check operation safely.
...
## Checklist
- [ ] Understand the exact goal of the data integrity check request.
...
## Example Usage
*Scenario:* An agent is tasked with data integrity check.
