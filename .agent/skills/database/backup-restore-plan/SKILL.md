...
type: skill
category: database
description: Plan backup, restore and recovery procedures before risky data changes.
when_to_use: When performing tasks related to backup restore plan.
do_not_use_when: When the task falls outside this domain or requires a different skill entirely.
risk_level: medium
owner_agents: [database-specialist, devops-specialist]
...
## Purpose
Plan backup, restore and recovery procedures before risky data changes.

## When to Use
Use this skill when tasked with executing a backup restore plan operation to ensure consistency and correctness.

## When Not to Use
Do not use this skill if the task does not involve backup restore plan, or if a more specific alternative skill exists.
...
## Procedure
1. Analyze the request and identify the specific requirements for backup restore plan.
2. Review existing patterns or codebase constraints before taking action.
3. Formulate a plan to execute the backup restore plan operation safely.
...
## Checklist
- [ ] Understand the exact goal of the backup restore plan request.
...
## Example Usage
*Scenario:* An agent is tasked with backup restore plan.
