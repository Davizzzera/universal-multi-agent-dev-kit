...
type: skill
category: database
description: Design tables, relationships, constraints and data models.
when_to_use: When performing tasks related to database schema design.
do_not_use_when: When the task falls outside this domain or requires a different skill entirely.
risk_level: medium
owner_agents: [database-specialist, software-architect]
...
## Purpose
Design tables, relationships, constraints and data models.

## When to Use
Use this skill when tasked with executing a database schema design operation to ensure consistency and correctness.

## When Not to Use
Do not use this skill if the task does not involve database schema design, or if a more specific alternative skill exists.
...
## Procedure
1. Analyze the request and identify the specific requirements for database schema design.
2. Review existing patterns or codebase constraints before taking action.
3. Formulate a plan to execute the database schema design operation safely.
...
## Checklist
- [ ] Understand the exact goal of the database schema design request.
...
## Example Usage
*Scenario:* An agent is tasked with database schema design.
