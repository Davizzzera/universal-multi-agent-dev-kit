...
type: skill
category: database
description: Review migrations for safety, compatibility and rollback readiness.
when_to_use: When performing tasks related to migration review.
do_not_use_when: When the task falls outside this domain or requires a different skill entirely.
risk_level: medium
owner_agents: [migration-specialist, technical-lead]
...
## Purpose
Review migrations for safety, compatibility and rollback readiness.

## When to Use
Use this skill when tasked with executing a migration review operation to ensure consistency and correctness.

## When Not to Use
Do not use this skill if the task does not involve migration review, or if a more specific alternative skill exists.
...
## Procedure
1. Analyze the request and identify the specific requirements for migration review.
2. Review existing patterns or codebase constraints before taking action.
3. Formulate a plan to execute the migration review operation safely.
...
## Checklist
- [ ] Understand the exact goal of the migration review request.
...
## Example Usage
*Scenario:* An agent is tasked with migration review.
