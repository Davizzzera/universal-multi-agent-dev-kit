...
type: skill
category: security
description: Review security readiness before release or deployment.
when_to_use: When performing tasks related to security release checklist.
do_not_use_when: When the task falls outside this domain or requires a different skill entirely.
risk_level: medium
owner_agents: [security-reviewer, release-manager]
...
## Purpose
Review security readiness before release or deployment.

## When to Use
Use this skill when tasked with executing a security release checklist operation to ensure consistency and correctness.

## When Not to Use
Do not use this skill if the task does not involve security release checklist, or if a more specific alternative skill exists.
...
## Procedure
1. Analyze the request and identify the specific requirements for security release checklist.
2. Review existing patterns or codebase constraints before taking action.
3. Formulate a plan to execute the security release checklist operation safely.
...
## Checklist
- [ ] Understand the exact goal of the security release checklist request.
...
## Example Usage
*Scenario:* An agent is tasked with security release checklist.
