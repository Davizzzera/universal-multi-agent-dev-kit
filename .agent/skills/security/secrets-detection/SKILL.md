...
type: skill
category: security
description: Detect accidental secrets, tokens, credentials or private keys.
when_to_use: When performing tasks related to secrets detection.
do_not_use_when: When the task falls outside this domain or requires a different skill entirely.
risk_level: medium
owner_agents: [secrets-scanner, security-reviewer]
...
## Purpose
Detect accidental secrets, tokens, credentials or private keys.

## When to Use
Use this skill when tasked with executing a secrets detection operation to ensure consistency and correctness.

## When Not to Use
Do not use this skill if the task does not involve secrets detection, or if a more specific alternative skill exists.
...
## Procedure
1. Analyze the request and identify the specific requirements for secrets detection.
2. Review existing patterns or codebase constraints before taking action.
3. Formulate a plan to execute the secrets detection operation safely.
...
## Checklist
- [ ] Understand the exact goal of the secrets detection request.
...
## Example Usage
*Scenario:* An agent is tasked with secrets detection.
