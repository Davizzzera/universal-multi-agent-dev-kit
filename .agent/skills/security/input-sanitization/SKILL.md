...
type: skill
category: security
description: Review and improve input validation and sanitization.
when_to_use: When performing tasks related to input sanitization.
do_not_use_when: When the task falls outside this domain or requires a different skill entirely.
risk_level: medium
owner_agents: [security-reviewer, backend-specialist]
...
## Purpose
Review and improve input validation and sanitization.

## When to Use
Use this skill when tasked with executing a input sanitization operation to ensure consistency and correctness.

## When Not to Use
Do not use this skill if the task does not involve input sanitization, or if a more specific alternative skill exists.
...
## Procedure
1. Analyze the request and identify the specific requirements for input sanitization.
2. Review existing patterns or codebase constraints before taking action.
3. Formulate a plan to execute the input sanitization operation safely.
...
## Checklist
- [ ] Understand the exact goal of the input sanitization request.
...
## Example Usage
*Scenario:* An agent is tasked with input sanitization.
