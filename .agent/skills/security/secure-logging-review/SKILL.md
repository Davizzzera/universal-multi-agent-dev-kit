---
name: secure-logging-review
type: skill
category: security
description: Ensure logs do not expose sensitive data and remain useful.
when_to_use: When performing tasks related to secure logging review.
do_not_use_when: When the task falls outside this domain or requires a different skill entirely.
risk_level: medium
owner_agents: [security-reviewer, observability style agents]
...
## Purpose
Ensure logs do not expose sensitive data and remain useful.

## When to Use
Use this skill when tasked with executing a secure logging review operation to ensure consistency and correctness.

## When Not to Use
Do not use this skill if the task does not involve secure logging review, or if a more specific alternative skill exists.
...
## Procedure
1. Analyze the request and identify the specific requirements for secure logging review.
2. Review existing patterns or codebase constraints before taking action.
3. Formulate a plan to execute the secure logging review operation safely.
...
## Checklist
- [ ] Understand the exact goal of the secure logging review request.
...
## Example Usage
*Scenario:* An agent is tasked with secure logging review.
