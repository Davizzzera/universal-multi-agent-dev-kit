...
type: skill
category: data-bi
description: Define reliable duplicate detection and removal rules.
when_to_use: When performing tasks related to deduplication rules.
do_not_use_when: When the task falls outside this domain or requires a different skill entirely.
risk_level: medium
owner_agents: [data-quality-specialist, business-analyst]
...
## Purpose
Define reliable duplicate detection and removal rules.

## When to Use
Use this skill when tasked with executing a deduplication rules operation to ensure consistency and correctness.

## When Not to Use
Do not use this skill if the task does not involve deduplication rules, or if a more specific alternative skill exists.
...
## Procedure
1. Analyze the request and identify the specific requirements for deduplication rules.
2. Review existing patterns or codebase constraints before taking action.
3. Formulate a plan to execute the deduplication rules operation safely.
...
## Checklist
- [ ] Understand the exact goal of the deduplication rules request.
...
## Example Usage
*Scenario:* An agent is tasked with deduplication rules.
