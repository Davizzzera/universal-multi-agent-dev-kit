...
type: skill
category: data-bi
description: Clean inconsistent, duplicated or invalid data.
when_to_use: When performing tasks related to data cleaning.
do_not_use_when: When the task falls outside this domain or requires a different skill entirely.
risk_level: medium
owner_agents: [data-quality-specialist, data-engineer style agents]
...
## Purpose
Clean inconsistent, duplicated or invalid data.

## When to Use
Use this skill when tasked with executing a data cleaning operation to ensure consistency and correctness.

## When Not to Use
Do not use this skill if the task does not involve data cleaning, or if a more specific alternative skill exists.
...
## Procedure
1. Analyze the request and identify the specific requirements for data cleaning.
2. Review existing patterns or codebase constraints before taking action.
3. Formulate a plan to execute the data cleaning operation safely.
...
## Checklist
- [ ] Understand the exact goal of the data cleaning request.
...
## Example Usage
*Scenario:* An agent is tasked with data cleaning.
