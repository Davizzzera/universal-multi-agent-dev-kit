...
type: skill
category: data-bi
description: Define dashboard objectives, visuals, filters and data expectations.
when_to_use: When performing tasks related to dashboard requirements.
do_not_use_when: When the task falls outside this domain or requires a different skill entirely.
risk_level: medium
owner_agents: [business-analyst, documentation-writer]
...
## Purpose
Define dashboard objectives, visuals, filters and data expectations.

## When to Use
Use this skill when tasked with executing a dashboard requirements operation to ensure consistency and correctness.

## When Not to Use
Do not use this skill if the task does not involve dashboard requirements, or if a more specific alternative skill exists.
...
## Procedure
1. Analyze the request and identify the specific requirements for dashboard requirements.
2. Review existing patterns or codebase constraints before taking action.
3. Formulate a plan to execute the dashboard requirements operation safely.
...
## Checklist
- [ ] Understand the exact goal of the dashboard requirements request.
...
## Example Usage
*Scenario:* An agent is tasked with dashboard requirements.
