---
name: csv-excel-ingestion
type: skill
category: data-bi
description: Ingest CSV and Excel files safely with validation and type handling.
when_to_use: When performing tasks related to csv excel ingestion.
do_not_use_when: When the task falls outside this domain or requires a different skill entirely.
risk_level: medium
owner_agents: [data-quality-specialist, python-automation-specialist]
...
## Purpose
Ingest CSV and Excel files safely with validation and type handling.

## When to Use
Use this skill when tasked with executing a csv excel ingestion operation to ensure consistency and correctness.

## When Not to Use
Do not use this skill if the task does not involve csv excel ingestion, or if a more specific alternative skill exists.
...
## Procedure
1. Analyze the request and identify the specific requirements for csv excel ingestion.
2. Review existing patterns or codebase constraints before taking action.
3. Formulate a plan to execute the csv excel ingestion operation safely.
...
## Checklist
- [ ] Understand the exact goal of the csv excel ingestion request.
...
## Example Usage
*Scenario:* An agent is tasked with csv excel ingestion.
