---
name: existing-patterns-detection
type: skill
category: context
description: Detect existing coding, architecture, naming and documentation patterns.
when_to_use: When performing tasks related to existing patterns detection.
do_not_use_when: When the task falls outside this domain or requires a different skill entirely.
risk_level: medium
owner_agents: [codebase-analyst, frontend-specialist, backend-specialist]
...
## Purpose
Detect existing coding, architecture, naming and documentation patterns.

## When to Use
Use this skill when tasked with executing a existing patterns detection operation to ensure consistency and correctness.

## When Not to Use
Do not use this skill if the task does not involve existing patterns detection, or if a more specific alternative skill exists.
...
## Procedure
1. Analyze the request and identify the specific requirements for existing patterns detection.
2. Review existing patterns or codebase constraints before taking action.
3. Formulate a plan to execute the existing patterns detection operation safely.
...
## Checklist
- [ ] Understand the exact goal of the existing patterns detection request.
...
## Example Usage
*Scenario:* An agent is tasked with existing patterns detection.
