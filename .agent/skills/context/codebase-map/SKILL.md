...
type: skill
category: context
description: Map folder structure, key files, modules and ownership areas.
when_to_use: When performing tasks related to codebase map.
do_not_use_when: When the task falls outside this domain or requires a different skill entirely.
risk_level: medium
owner_agents: [project-context-reader, codebase-analyst, technical-lead]
...
## Purpose
Map folder structure, key files, modules and ownership areas.

## When to Use
Use this skill when tasked with executing a codebase map operation to ensure consistency and correctness.

## When Not to Use
Do not use this skill if the task does not involve codebase map, or if a more specific alternative skill exists.
...
## Procedure
1. Analyze the request and identify the specific requirements for codebase map.
2. Review existing patterns or codebase constraints before taking action.
3. Formulate a plan to execute the codebase map operation safely.
...
## Checklist
- [ ] Understand the exact goal of the codebase map request.
...
## Example Usage
*Scenario:* An agent is tasked with codebase map.
