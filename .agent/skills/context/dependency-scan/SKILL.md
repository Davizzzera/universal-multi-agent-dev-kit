...
type: skill
category: context
description: Identify package managers, dependencies, frameworks and runtime constraints.
when_to_use: When performing tasks related to dependency scan.
do_not_use_when: When the task falls outside this domain or requires a different skill entirely.
risk_level: medium
owner_agents: [project-context-reader, dependency-security-auditor, devops-specialist]
...
## Purpose
Identify package managers, dependencies, frameworks and runtime constraints.

## When to Use
Use this skill when tasked with executing a dependency scan operation to ensure consistency and correctness.

## When Not to Use
Do not use this skill if the task does not involve dependency scan, or if a more specific alternative skill exists.
...
## Procedure
1. Analyze the request and identify the specific requirements for dependency scan.
2. Review existing patterns or codebase constraints before taking action.
3. Formulate a plan to execute the dependency scan operation safely.
...
## Checklist
- [ ] Understand the exact goal of the dependency scan request.
...
## Example Usage
*Scenario:* An agent is tasked with dependency scan.
