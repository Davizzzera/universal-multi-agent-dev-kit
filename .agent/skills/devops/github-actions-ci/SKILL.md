...
type: skill
category: devops
description: Create GitHub Actions pipelines for validation and CI.
when_to_use: When performing tasks related to github actions ci.
do_not_use_when: When the task falls outside this domain or requires a different skill entirely.
risk_level: medium
owner_agents: [ci-cd-specialist]
...
## Purpose
Create GitHub Actions pipelines for validation and CI.

## When to Use
Use this skill when tasked with executing a github actions ci operation to ensure consistency and correctness.

## When Not to Use
Do not use this skill if the task does not involve github actions ci, or if a more specific alternative skill exists.
...
## Procedure
1. Analyze the request and identify the specific requirements for github actions ci.
2. Review existing patterns or codebase constraints before taking action.
3. Formulate a plan to execute the github actions ci operation safely.
...
## Checklist
- [ ] Understand the exact goal of the github actions ci request.
...
## Example Usage
*Scenario:* An agent is tasked with github actions ci.
