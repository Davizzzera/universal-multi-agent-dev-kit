...
type: skill
category: devops
description: Create docker-compose configurations for local or multi-service environments.
when_to_use: When performing tasks related to docker compose create.
do_not_use_when: When the task falls outside this domain or requires a different skill entirely.
risk_level: medium
owner_agents: [docker-specialist, devops-specialist]
...
## Purpose
Create docker-compose configurations for local or multi-service environments.

## When to Use
Use this skill when tasked with executing a docker compose create operation to ensure consistency and correctness.

## When Not to Use
Do not use this skill if the task does not involve docker compose create, or if a more specific alternative skill exists.
...
## Procedure
1. Analyze the request and identify the specific requirements for docker compose create.
2. Review existing patterns or codebase constraints before taking action.
3. Formulate a plan to execute the docker compose create operation safely.
...
## Checklist
- [ ] Understand the exact goal of the docker compose create request.
...
## Example Usage
*Scenario:* An agent is tasked with docker compose create.
