...
type: skill
category: devops
description: Validate whether a project is ready for production usage.
when_to_use: When performing tasks related to production readiness check.
do_not_use_when: When the task falls outside this domain or requires a different skill entirely.
risk_level: medium
owner_agents: [devops-specialist, security-reviewer, release-manager]
...
## Purpose
Validate whether a project is ready for production usage.

## When to Use
Use this skill when tasked with executing a production readiness check operation to ensure consistency and correctness.

## When Not to Use
Do not use this skill if the task does not involve production readiness check, or if a more specific alternative skill exists.
...
## Procedure
1. Analyze the request and identify the specific requirements for production readiness check.
2. Review existing patterns or codebase constraints before taking action.
3. Formulate a plan to execute the production readiness check operation safely.
...
## Checklist
- [ ] Understand the exact goal of the production readiness check request.
...
## Example Usage
*Scenario:* An agent is tasked with production readiness check.
