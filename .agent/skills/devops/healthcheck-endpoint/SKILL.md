...
type: skill
category: devops
description: Define healthcheck behavior for services and deployments.
when_to_use: When performing tasks related to healthcheck endpoint.
do_not_use_when: When the task falls outside this domain or requires a different skill entirely.
risk_level: medium
owner_agents: [backend-specialist, devops-specialist]
...
## Purpose
Define healthcheck behavior for services and deployments.

## When to Use
Use this skill when tasked with executing a healthcheck endpoint operation to ensure consistency and correctness.

## When Not to Use
Do not use this skill if the task does not involve healthcheck endpoint, or if a more specific alternative skill exists.
...
## Procedure
1. Analyze the request and identify the specific requirements for healthcheck endpoint.
2. Review existing patterns or codebase constraints before taking action.
3. Formulate a plan to execute the healthcheck endpoint operation safely.
...
## Checklist
- [ ] Understand the exact goal of the healthcheck endpoint request.
...
## Example Usage
*Scenario:* An agent is tasked with healthcheck endpoint.
