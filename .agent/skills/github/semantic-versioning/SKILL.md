...
type: skill
category: github
description: Apply semantic versioning rules to releases.
when_to_use: When performing tasks related to semantic versioning.
do_not_use_when: When the task falls outside this domain or requires a different skill entirely.
risk_level: medium
owner_agents: [release-manager, changelog-maintainer]
...
## When to Use
Use this skill when tasked with executing a semantic versioning operation to ensure consistency and correctness.

## When Not to Use
Do not use this skill if the task does not involve semantic versioning, or if a more specific alternative skill exists.
...
## Procedure
1. Analyze the request and identify the specific requirements for semantic versioning.
2. Review existing patterns or codebase constraints before taking action.
3. Formulate a plan to execute the semantic versioning operation safely.
...
## Checklist
- [ ] Understand the exact goal of the semantic versioning request.
...
## Example Usage
*Scenario:* An agent is tasked with semantic versioning.
