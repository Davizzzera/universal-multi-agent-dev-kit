...
type: skill
category: github
description: Update changelog entries accurately and consistently.
when_to_use: When performing tasks related to changelog update.
do_not_use_when: When the task falls outside this domain or requires a different skill entirely.
risk_level: medium
owner_agents: [changelog-maintainer, release-manager]
...
## Purpose
Update changelog entries accurately and consistently.

## When to Use
Use this skill when tasked with executing a changelog update operation to ensure consistency and correctness.

## When Not to Use
Do not use this skill if the task does not involve changelog update, or if a more specific alternative skill exists.
...
## Procedure
1. Analyze the request and identify the specific requirements for changelog update.
2. Review existing patterns or codebase constraints before taking action.
3. Formulate a plan to execute the changelog update operation safely.
...
## Checklist
- [ ] Understand the exact goal of the changelog update request.
...
## Example Usage
*Scenario:* An agent is tasked with changelog update.
