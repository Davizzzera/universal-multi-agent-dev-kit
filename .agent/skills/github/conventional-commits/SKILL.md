...
type: skill
category: github
description: Use clear Conventional Commit messages.
when_to_use: When performing tasks related to conventional commits.
do_not_use_when: When the task falls outside this domain or requires a different skill entirely.
risk_level: medium
owner_agents: [release-manager, github-repository-manager]
...
## When to Use
Use this skill when tasked with executing a conventional commits operation to ensure consistency and correctness.

## When Not to Use
Do not use this skill if the task does not involve conventional commits, or if a more specific alternative skill exists.
...
## Procedure
1. Analyze the request and identify the specific requirements for conventional commits.
2. Review existing patterns or codebase constraints before taking action.
3. Formulate a plan to execute the conventional commits operation safely.
...
## Checklist
- [ ] Understand the exact goal of the conventional commits request.
...
## Example Usage
*Scenario:* An agent is tasked with conventional commits.
