---
name: n8n-webhook-flow
type: skill
category: automation
description: Design n8n webhook-based automations with validation and error handling.
when_to_use: When performing tasks related to n8n webhook flow.
do_not_use_when: When the task falls outside this domain or requires a different skill entirely.
risk_level: medium
owner_agents: [n8n-automation-specialist, webhook-specialist]
...
## Purpose
Design n8n webhook-based automations with validation and error handling.

## When to Use
Use this skill when tasked with executing a n8n webhook flow operation to ensure consistency and correctness.

## When Not to Use
Do not use this skill if the task does not involve n8n webhook flow, or if a more specific alternative skill exists.
...
## Procedure
1. Analyze the request and identify the specific requirements for n8n webhook flow.
2. Review existing patterns or codebase constraints before taking action.
3. Formulate a plan to execute the n8n webhook flow operation safely.
...
## Checklist
- [ ] Understand the exact goal of the n8n webhook flow request.
...
## Example Usage
*Scenario:* An agent is tasked with n8n webhook flow.
