...
type: skill
category: automation
description: Create safe browser automations using Playwright patterns.
when_to_use: When performing tasks related to browser automation playwright.
do_not_use_when: When the task falls outside this domain or requires a different skill entirely.
risk_level: medium
owner_agents: [python-automation-specialist, qa-tester]
...
## Purpose
Create safe browser automations using Playwright patterns.

## When to Use
Use this skill when tasked with executing a browser automation playwright operation to ensure consistency and correctness.

## When Not to Use
Do not use this skill if the task does not involve browser automation playwright, or if a more specific alternative skill exists.
...
## Procedure
1. Analyze the request and identify the specific requirements for browser automation playwright.
2. Review existing patterns or codebase constraints before taking action.
3. Formulate a plan to execute the browser automation playwright operation safely.
...
## Checklist
- [ ] Understand the exact goal of the browser automation playwright request.
...
## Example Usage
*Scenario:* An agent is tasked with browser automation playwright.
