---
name: error-retry-strategy
type: skill
category: automation
description: Define retry, backoff, idempotency and error recovery behavior.
when_to_use: When performing tasks related to error retry strategy.
do_not_use_when: When the task falls outside this domain or requires a different skill entirely.
risk_level: medium
owner_agents: [integration-specialist, backend-specialist, n8n-automation-specialist]
...
## Purpose
Define retry, backoff, idempotency and error recovery behavior.

## When to Use
Use this skill when tasked with executing a error retry strategy operation to ensure consistency and correctness.

## When Not to Use
Do not use this skill if the task does not involve error retry strategy, or if a more specific alternative skill exists.
...
## Procedure
1. Analyze the request and identify the specific requirements for error retry strategy.
2. Review existing patterns or codebase constraints before taking action.
3. Formulate a plan to execute the error retry strategy operation safely.
...
## Checklist
- [ ] Understand the exact goal of the error retry strategy request.
...
## Example Usage
*Scenario:* An agent is tasked with error retry strategy.
