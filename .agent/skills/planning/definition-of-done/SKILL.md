...
type: skill
category: planning
description: Define what must be true before a task can be considered complete.
when_to_use: When performing tasks related to definition of done.
do_not_use_when: When the task falls outside this domain or requires a different skill entirely.
risk_level: medium
owner_agents: [final-reviewer, qa-tester, product-manager]
...
## Purpose
Define what must be true before a task can be considered complete.

## When to Use
Use this skill when tasked with executing a definition of done operation to ensure consistency and correctness.

## When Not to Use
Do not use this skill if the task does not involve definition of done, or if a more specific alternative skill exists.
...
## Procedure
1. Analyze the request and identify the specific requirements for definition of done.
2. Review existing patterns or codebase constraints before taking action.
3. Formulate a plan to execute the definition of done operation safely.
...
## Checklist
- [ ] Understand the exact goal of the definition of done request.
...
## Example Usage
*Scenario:* An agent is tasked with definition of done.
