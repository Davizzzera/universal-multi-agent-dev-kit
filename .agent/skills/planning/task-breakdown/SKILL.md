...
type: skill
category: planning
description: Break a broad request into clear, actionable subtasks.
when_to_use: When performing tasks related to task breakdown.
do_not_use_when: When the task falls outside this domain or requires a different skill entirely.
risk_level: medium
owner_agents: [universal-orchestrator, task-router, technical-lead]
...
## Purpose
Break a broad request into clear, actionable subtasks.

## When to Use
Use this skill when tasked with executing a task breakdown operation to ensure consistency and correctness.

## When Not to Use
Do not use this skill if the task does not involve task breakdown, or if a more specific alternative skill exists.
...
## Procedure
1. Analyze the request and identify the specific requirements for task breakdown.
2. Review existing patterns or codebase constraints before taking action.
3. Formulate a plan to execute the task breakdown operation safely.
...
## Checklist
- [ ] Understand the exact goal of the task breakdown request.
...
## Example Usage
*Scenario:* An agent is tasked with task breakdown.
