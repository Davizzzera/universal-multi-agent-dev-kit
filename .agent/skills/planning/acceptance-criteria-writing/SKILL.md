...
type: skill
category: planning
description: Define measurable acceptance criteria for a task or feature.
when_to_use: When performing tasks related to acceptance criteria writing.
do_not_use_when: When the task falls outside this domain or requires a different skill entirely.
risk_level: medium
owner_agents: [requirements-engineer, product-manager, qa-tester]
...
## Purpose
Define measurable acceptance criteria for a task or feature.

## When to Use
Use this skill when tasked with executing a acceptance criteria writing operation to ensure consistency and correctness.

## When Not to Use
Do not use this skill if the task does not involve acceptance criteria writing, or if a more specific alternative skill exists.
...
## Procedure
1. Analyze the request and identify the specific requirements for acceptance criteria writing.
2. Review existing patterns or codebase constraints before taking action.
3. Formulate a plan to execute the acceptance criteria writing operation safely.
...
## Checklist
- [ ] Understand the exact goal of the acceptance criteria writing request.
...
## Example Usage
*Scenario:* An agent is tasked with acceptance criteria writing.
