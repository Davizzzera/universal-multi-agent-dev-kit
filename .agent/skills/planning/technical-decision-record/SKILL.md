...
type: skill
category: planning
description: Document important technical decisions and tradeoffs.
when_to_use: When performing tasks related to technical decision record.
do_not_use_when: When the task falls outside this domain or requires a different skill entirely.
risk_level: medium
owner_agents: [software-architect, technical-writer, technical-lead]
...
## Purpose
Document important technical decisions and tradeoffs.

## When to Use
Use this skill when tasked with executing a technical decision record operation to ensure consistency and correctness.

## When Not to Use
Do not use this skill if the task does not involve technical decision record, or if a more specific alternative skill exists.
...
## Procedure
1. Analyze the request and identify the specific requirements for technical decision record.
2. Review existing patterns or codebase constraints before taking action.
3. Formulate a plan to execute the technical decision record operation safely.
...
## Checklist
- [ ] Understand the exact goal of the technical decision record request.
...
## Example Usage
*Scenario:* An agent is tasked with technical decision record.
