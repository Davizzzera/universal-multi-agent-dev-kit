...
type: skill
category: planning
description: Plan how to revert or safely recover from a change.
when_to_use: When performing tasks related to rollback plan.
do_not_use_when: When the task falls outside this domain or requires a different skill entirely.
risk_level: medium
owner_agents: [technical-lead, devops-specialist, database-specialist]
...
## Purpose
Plan how to revert or safely recover from a change.

## When to Use
Use this skill when tasked with executing a rollback plan operation to ensure consistency and correctness.

## When Not to Use
Do not use this skill if the task does not involve rollback plan, or if a more specific alternative skill exists.
...
## Procedure
1. Analyze the request and identify the specific requirements for rollback plan.
2. Review existing patterns or codebase constraints before taking action.
3. Formulate a plan to execute the rollback plan operation safely.
...
## Checklist
- [ ] Understand the exact goal of the rollback plan request.
...
## Example Usage
*Scenario:* An agent is tasked with rollback plan.
