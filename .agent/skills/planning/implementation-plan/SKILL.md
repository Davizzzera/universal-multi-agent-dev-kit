...
type: skill
category: planning
description: Create a safe, scoped and ordered implementation plan.
when_to_use: When performing tasks related to implementation plan.
do_not_use_when: When the task falls outside this domain or requires a different skill entirely.
risk_level: medium
owner_agents: [universal-orchestrator, technical-lead, software-architect]
...
## Purpose
Create a safe, scoped and ordered implementation plan.

## When to Use
Use this skill when tasked with executing a implementation plan operation to ensure consistency and correctness.

## When Not to Use
Do not use this skill if the task does not involve implementation plan, or if a more specific alternative skill exists.
...
## Procedure
1. Analyze the request and identify the specific requirements for implementation plan.
2. Review existing patterns or codebase constraints before taking action.
3. Formulate a plan to execute the implementation plan operation safely.
...
## Checklist
- [ ] Understand the exact goal of the implementation plan request.
...
## Example Usage
*Scenario:* An agent is tasked with implementation plan.
