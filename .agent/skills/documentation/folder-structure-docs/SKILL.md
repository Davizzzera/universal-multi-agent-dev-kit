---
name: folder-structure-docs
type: skill
category: documentation
description: Document folder structure and file responsibilities.
when_to_use: When performing tasks related to folder structure docs.
do_not_use_when: When the task falls outside this domain or requires a different skill entirely.
risk_level: medium
owner_agents: [documentation-writer, github-repository-manager]
...
## Purpose
Document folder structure and file responsibilities.

## When to Use
Use this skill when tasked with executing a folder structure docs operation to ensure consistency and correctness.

## When Not to Use
Do not use this skill if the task does not involve folder structure docs, or if a more specific alternative skill exists.
...
## Procedure
1. Analyze the request and identify the specific requirements for folder structure docs.
2. Review existing patterns or codebase constraints before taking action.
3. Formulate a plan to execute the folder structure docs operation safely.
...
## Checklist
- [ ] Understand the exact goal of the folder structure docs request.
...
## Example Usage
*Scenario:* An agent is tasked with folder structure docs.
