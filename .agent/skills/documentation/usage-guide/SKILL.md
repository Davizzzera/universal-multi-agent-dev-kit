...
description: Document how to use a tool, feature or repository.
when_to_use: When performing tasks related to usage guide.
do_not_use_when: When the task falls outside this domain or requires a different skill entirely.
risk_level: medium
owner_agents: [documentation-writer]
...
## Purpose
Document how to use a tool, feature or repository.

## When to Use
Use this skill when tasked with executing a usage guide operation to ensure consistency and correctness.

## When Not to Use
Do not use this skill if the task does not involve usage guide, or if a more specific alternative skill exists.
...
## Procedure
1. Analyze the request and identify the specific requirements for usage guide.
2. Review existing patterns or codebase constraints before taking action.
3. Formulate a plan to execute the usage guide operation safely.
...
## Checklist
- [ ] Understand the exact goal of the usage guide request.
...
## Example Usage
*Scenario:* An agent is tasked with usage guide.
