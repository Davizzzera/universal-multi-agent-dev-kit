...
type: skill
category: auth
description: Define and review role-based permission systems.
when_to_use: When performing tasks related to role based access control.
do_not_use_when: When the task falls outside this domain or requires a different skill entirely.
risk_level: medium
owner_agents: [auth-specialist, auth-security-reviewer]
...
## Purpose
Define and review role-based permission systems.

## When to Use
Use this skill when tasked with executing a role based access control operation to ensure consistency and correctness.

## When Not to Use
Do not use this skill if the task does not involve role based access control, or if a more specific alternative skill exists.
...
## Procedure
1. Analyze the request and identify the specific requirements for role based access control.
2. Review existing patterns or codebase constraints before taking action.
3. Formulate a plan to execute the role based access control operation safely.
...
## Checklist
- [ ] Understand the exact goal of the role based access control request.
...
## Example Usage
*Scenario:* An agent is tasked with role based access control.
