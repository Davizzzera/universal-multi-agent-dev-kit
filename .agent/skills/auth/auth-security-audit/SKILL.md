...
type: skill
category: auth
description: Audit authentication and authorization flows defensively.
when_to_use: When performing tasks related to auth security audit.
do_not_use_when: When the task falls outside this domain or requires a different skill entirely.
risk_level: medium
owner_agents: [auth-security-reviewer, security-reviewer]
...
## Purpose
Audit authentication and authorization flows defensively.

## When to Use
Use this skill when tasked with executing a auth security audit operation to ensure consistency and correctness.

## When Not to Use
Do not use this skill if the task does not involve auth security audit, or if a more specific alternative skill exists.
...
## Procedure
1. Analyze the request and identify the specific requirements for auth security audit.
2. Review existing patterns or codebase constraints before taking action.
3. Formulate a plan to execute the auth security audit operation safely.
...
## Checklist
- [ ] Understand the exact goal of the auth security audit request.
...
## Example Usage
*Scenario:* An agent is tasked with auth security audit.
