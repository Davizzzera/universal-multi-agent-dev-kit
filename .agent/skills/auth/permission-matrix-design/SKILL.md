---
name: permission-matrix-design
type: skill
category: auth
description: Create a clear permissions matrix for roles, features and actions.
when_to_use: When performing tasks related to permission matrix design.
do_not_use_when: When the task falls outside this domain or requires a different skill entirely.
risk_level: medium
owner_agents: [auth-specialist, business-analyst, security-reviewer]
...
## Purpose
Create a clear permissions matrix for roles, features and actions.

## When to Use
Use this skill when tasked with executing a permission matrix design operation to ensure consistency and correctness.

## When Not to Use
Do not use this skill if the task does not involve permission matrix design, or if a more specific alternative skill exists.
...
## Procedure
1. Analyze the request and identify the specific requirements for permission matrix design.
2. Review existing patterns or codebase constraints before taking action.
3. Formulate a plan to execute the permission matrix design operation safely.
...
## Checklist
- [ ] Understand the exact goal of the permission matrix design request.
...
## Example Usage
*Scenario:* An agent is tasked with permission matrix design.
