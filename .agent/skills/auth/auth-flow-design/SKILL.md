...
type: skill
category: auth
description: Design authentication and session flows safely.
when_to_use: When performing tasks related to auth flow design.
...
## Purpose
Design authentication and session flows safely.

## When to Use
Use this skill when tasked with executing a auth flow design operation to ensure consistency and correctness.

## When Not to Use
Do not use this skill if the task does not involve auth flow design, or if a more specific alternative skill exists.
...
## Procedure
1. Analyze the request and identify the specific requirements for auth flow design.
2. Review existing patterns or codebase constraints before taking action.
3. Formulate a plan to execute the auth flow design operation safely.
...
## Checklist
- [ ] Understand the exact goal of the auth flow design request.
...
## Example Usage
*Scenario:* An agent is tasked with auth flow design.
