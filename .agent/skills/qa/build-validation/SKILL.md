...
type: skill
category: qa
description: Validate that the project builds successfully after changes.
when_to_use: When performing tasks related to build validation.
do_not_use_when: When the task falls outside this domain or requires a different skill entirely.
risk_level: medium
owner_agents: [qa-tester, devops-specialist]
...
## Purpose
Validate that the project builds successfully after changes.

## When to Use
Use this skill when tasked with executing a build validation operation to ensure consistency and correctness.

## When Not to Use
Do not use this skill if the task does not involve build validation, or if a more specific alternative skill exists.
...
## Procedure
1. Analyze the request and identify the specific requirements for build validation.
2. Review existing patterns or codebase constraints before taking action.
3. Formulate a plan to execute the build validation operation safely.
...
## Checklist
- [ ] Understand the exact goal of the build validation request.
...
## Example Usage
*Scenario:* An agent is tasked with build validation.
