...
type: skill
category: qa
description: Create or improve unit tests for isolated logic.
when_to_use: When performing tasks related to unit test create.
do_not_use_when: When the task falls outside this domain or requires a different skill entirely.
risk_level: medium
owner_agents: [unit-test-specialist]
...
## Purpose
Create or improve unit tests for isolated logic.

## When to Use
Use this skill when tasked with executing a unit test create operation to ensure consistency and correctness.

## When Not to Use
Do not use this skill if the task does not involve unit test create, or if a more specific alternative skill exists.
...
## Procedure
1. Analyze the request and identify the specific requirements for unit test create.
2. Review existing patterns or codebase constraints before taking action.
3. Formulate a plan to execute the unit test create operation safely.
...
## Checklist
- [ ] Understand the exact goal of the unit test create request.
...
## Example Usage
*Scenario:* An agent is tasked with unit test create.
