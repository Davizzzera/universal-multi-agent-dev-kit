...
type: skill
category: qa
description: Create a checklist to verify existing behavior was not broken.
when_to_use: When performing tasks related to regression test checklist.
do_not_use_when: When the task falls outside this domain or requires a different skill entirely.
risk_level: medium
owner_agents: [qa-tester, final-reviewer]
...
## Purpose
Create a checklist to verify existing behavior was not broken.

## When to Use
Use this skill when tasked with executing a regression test checklist operation to ensure consistency and correctness.

## When Not to Use
Do not use this skill if the task does not involve regression test checklist, or if a more specific alternative skill exists.
...
## Procedure
1. Analyze the request and identify the specific requirements for regression test checklist.
2. Review existing patterns or codebase constraints before taking action.
3. Formulate a plan to execute the regression test checklist operation safely.
...
## Checklist
- [ ] Understand the exact goal of the regression test checklist request.
...
## Example Usage
*Scenario:* An agent is tasked with regression test checklist.
