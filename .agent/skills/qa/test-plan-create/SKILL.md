...
type: skill
category: qa
description: Create a validation plan based on requirements and risk.
when_to_use: When performing tasks related to test plan create.
do_not_use_when: When the task falls outside this domain or requires a different skill entirely.
risk_level: medium
owner_agents: [qa-tester]
...
## Purpose
Create a validation plan based on requirements and risk.

## When to Use
Use this skill when tasked with executing a test plan create operation to ensure consistency and correctness.

## When Not to Use
Do not use this skill if the task does not involve test plan create, or if a more specific alternative skill exists.
...
## Procedure
1. Analyze the request and identify the specific requirements for test plan create.
2. Review existing patterns or codebase constraints before taking action.
3. Formulate a plan to execute the test plan create operation safely.
...
## Checklist
- [ ] Understand the exact goal of the test plan create request.
...
## Example Usage
*Scenario:* An agent is tasked with test plan create.
