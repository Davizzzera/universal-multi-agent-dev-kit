...
type: skill
category: qa
description: Create tests that validate interactions between modules or services.
when_to_use: When performing tasks related to integration test create.
do_not_use_when: When the task falls outside this domain or requires a different skill entirely.
risk_level: medium
owner_agents: [qa-tester, backend-specialist]
...
## Purpose
Create tests that validate interactions between modules or services.

## When to Use
Use this skill when tasked with executing a integration test create operation to ensure consistency and correctness.

## When Not to Use
Do not use this skill if the task does not involve integration test create, or if a more specific alternative skill exists.
...
## Procedure
1. Analyze the request and identify the specific requirements for integration test create.
2. Review existing patterns or codebase constraints before taking action.
3. Formulate a plan to execute the integration test create operation safely.
...
## Checklist
- [ ] Understand the exact goal of the integration test create request.
...
## Example Usage
*Scenario:* An agent is tasked with integration test create.
