...
description: Coordinate multi-agent execution from request interpretation to final synthesis.
when_to_use: When performing tasks related to coordinator mode.
do_not_use_when: When the task falls outside this domain or requires a different skill entirely.
risk_level: medium
owner_agents: [universal-orchestrator, task-router, conflict-controller]
...
## Purpose
Coordinate multi-agent execution from request interpretation to final synthesis.

## When to Use
Use this skill when tasked with executing a coordinator mode operation to ensure consistency and correctness.

## When Not to Use
Do not use this skill if the task does not involve coordinator mode, or if a more specific alternative skill exists.
...
## Procedure
1. Analyze the request and identify the specific requirements for coordinator mode.
2. Review existing patterns or codebase constraints before taking action.
3. Formulate a plan to execute the coordinator mode operation safely.
...
## Checklist
- [ ] Understand the exact goal of the coordinator mode request.
...
## Example Usage
*Scenario:* An agent is tasked with coordinator mode.
