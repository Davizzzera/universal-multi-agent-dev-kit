---
name: file-ownership-locking
type: skill
category: orchestration
description: Assign file ownership and prevent simultaneous conflicting edits.
when_to_use: When performing tasks related to file ownership locking.
do_not_use_when: When the task falls outside this domain or requires a different skill entirely.
risk_level: medium
owner_agents: [conflict-controller, scope-guardian, technical-lead]
...
## Purpose
Assign file ownership and prevent simultaneous conflicting edits.

## When to Use
Use this skill when tasked with executing a file ownership locking operation to ensure consistency and correctness.

## When Not to Use
Do not use this skill if the task does not involve file ownership locking, or if a more specific alternative skill exists.
...
## Procedure
1. Analyze the request and identify the specific requirements for file ownership locking.
2. Review existing patterns or codebase constraints before taking action.
3. Formulate a plan to execute the file ownership locking operation safely.
...
## Checklist
- [ ] Understand the exact goal of the file ownership locking request.
...
## Example Usage
*Scenario:* An agent is tasked with file ownership locking.
