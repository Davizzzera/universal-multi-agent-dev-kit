...
description: Separate tasks that can run in parallel from tasks that must run sequentially.
when_to_use: When performing tasks related to parallel work planning.
do_not_use_when: When the task falls outside this domain or requires a different skill entirely.
risk_level: medium
owner_agents: [conflict-controller, universal-orchestrator, technical-lead]
...
## Purpose
Separate tasks that can run in parallel from tasks that must run sequentially.

## When to Use
Use this skill when tasked with executing a parallel work planning operation to ensure consistency and correctness.

## When Not to Use
Do not use this skill if the task does not involve parallel work planning, or if a more specific alternative skill exists.
...
## Procedure
1. Analyze the request and identify the specific requirements for parallel work planning.
2. Review existing patterns or codebase constraints before taking action.
3. Formulate a plan to execute the parallel work planning operation safely.
...
## Checklist
- [ ] Understand the exact goal of the parallel work planning request.
...
## Example Usage
*Scenario:* An agent is tasked with parallel work planning.
