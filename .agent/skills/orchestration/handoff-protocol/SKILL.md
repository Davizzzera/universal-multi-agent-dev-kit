...
description: Transfer work safely between agents with complete context.
when_to_use: When performing tasks related to handoff protocol.
do_not_use_when: When the task falls outside this domain or requires a different skill entirely.
risk_level: medium
owner_agents: [universal-orchestrator, final-reviewer, all specialist agents]
...
## Purpose
Transfer work safely between agents with complete context.

## When to Use
Use this skill when tasked with executing a handoff protocol operation to ensure consistency and correctness.

## When Not to Use
Do not use this skill if the task does not involve handoff protocol, or if a more specific alternative skill exists.
...
## Procedure
1. Analyze the request and identify the specific requirements for handoff protocol.
2. Review existing patterns or codebase constraints before taking action.
3. Formulate a plan to execute the handoff protocol operation safely.
...
## Checklist
- [ ] Understand the exact goal of the handoff protocol request.
...
## Example Usage
*Scenario:* An agent is tasked with handoff protocol.
