...
type: skill
category: frontend
description: Review local and shared frontend state for simplicity and correctness.
when_to_use: When performing tasks related to state management review.
do_not_use_when: When the task falls outside this domain or requires a different skill entirely.
risk_level: medium
owner_agents: [react-specialist, frontend-specialist, technical-lead]
...
## Purpose
Review local and shared frontend state for simplicity and correctness.

## When to Use
Use this skill when tasked with executing a state management review operation to ensure consistency and correctness.

## When Not to Use
Do not use this skill if the task does not involve state management review, or if a more specific alternative skill exists.
...
## Procedure
1. Analyze the request and identify the specific requirements for state management review.
2. Review existing patterns or codebase constraints before taking action.
3. Formulate a plan to execute the state management review operation safely.
...
## Checklist
- [ ] Understand the exact goal of the state management review request.
...
## Example Usage
*Scenario:* An agent is tasked with state management review.
