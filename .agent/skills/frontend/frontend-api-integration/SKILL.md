...
type: skill
category: frontend
description: Connect frontend components to APIs safely and consistently.
when_to_use: When performing tasks related to frontend api integration.
do_not_use_when: When the task falls outside this domain or requires a different skill entirely.
risk_level: medium
owner_agents: [frontend-specialist, react-specialist, api-designer]
...
## Purpose
Connect frontend components to APIs safely and consistently.

## When to Use
Use this skill when tasked with executing a frontend api integration operation to ensure consistency and correctness.

## When Not to Use
Do not use this skill if the task does not involve frontend api integration, or if a more specific alternative skill exists.
...
## Procedure
1. Analyze the request and identify the specific requirements for frontend api integration.
2. Review existing patterns or codebase constraints before taking action.
3. Formulate a plan to execute the frontend api integration operation safely.
...
## Checklist
- [ ] Understand the exact goal of the frontend api integration request.
...
## Example Usage
*Scenario:* An agent is tasked with frontend api integration.
