...
type: skill
category: frontend
description: Improve React components without changing external behavior.
when_to_use: When performing tasks related to react component refactor.
do_not_use_when: When the task falls outside this domain or requires a different skill entirely.
risk_level: medium
owner_agents: [react-specialist, refactor-specialist]
...
## Purpose
Improve React components without changing external behavior.

## When to Use
Use this skill when tasked with executing a react component refactor operation to ensure consistency and correctness.

## When Not to Use
Do not use this skill if the task does not involve react component refactor, or if a more specific alternative skill exists.
...
## Procedure
1. Analyze the request and identify the specific requirements for react component refactor.
2. Review existing patterns or codebase constraints before taking action.
3. Formulate a plan to execute the react component refactor operation safely.
...
## Checklist
- [ ] Understand the exact goal of the react component refactor request.
...
## Example Usage
*Scenario:* An agent is tasked with react component refactor.
