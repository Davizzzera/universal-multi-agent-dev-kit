...
type: skill
category: frontend
description: Implement user-friendly loading, empty and error states.
when_to_use: When performing tasks related to loading empty error states.
do_not_use_when: When the task falls outside this domain or requires a different skill entirely.
risk_level: medium
owner_agents: [frontend-specialist, ui-ux-designer]
...
## Purpose
Implement user-friendly loading, empty and error states.

## When to Use
Use this skill when tasked with executing a loading empty error states operation to ensure consistency and correctness.

## When Not to Use
Do not use this skill if the task does not involve loading empty error states, or if a more specific alternative skill exists.
...
## Procedure
1. Analyze the request and identify the specific requirements for loading empty error states.
2. Review existing patterns or codebase constraints before taking action.
3. Formulate a plan to execute the loading empty error states operation safely.
...
## Checklist
- [ ] Understand the exact goal of the loading empty error states request.
...
## Example Usage
*Scenario:* An agent is tasked with loading empty error states.
