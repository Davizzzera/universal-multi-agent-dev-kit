...
type: skill
category: backend
description: Validate incoming data before processing it.
when_to_use: When performing tasks related to request validation.
do_not_use_when: When the task falls outside this domain or requires a different skill entirely.
risk_level: medium
owner_agents: [backend-specialist, security-reviewer]
...
## When to Use
Use this skill when tasked with executing a request validation operation to ensure consistency and correctness.

## When Not to Use
Do not use this skill if the task does not involve request validation, or if a more specific alternative skill exists.
...
## Procedure
1. Analyze the request and identify the specific requirements for request validation.
2. Review existing patterns or codebase constraints before taking action.
3. Formulate a plan to execute the request validation operation safely.
...
## Checklist
- [ ] Understand the exact goal of the request validation request.
...
## Example Usage
*Scenario:* An agent is tasked with request validation.
