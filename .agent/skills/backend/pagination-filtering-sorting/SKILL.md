...
type: skill
category: backend
description: Implement list endpoints with pagination, filtering and sorting.
when_to_use: When performing tasks related to pagination filtering sorting.
do_not_use_when: When the task falls outside this domain or requires a different skill entirely.
risk_level: medium
owner_agents: [backend-specialist, api-designer]
...
## Purpose
Implement list endpoints with pagination, filtering and sorting.

## When to Use
Use this skill when tasked with executing a pagination filtering sorting operation to ensure consistency and correctness.

## When Not to Use
Do not use this skill if the task does not involve pagination filtering sorting, or if a more specific alternative skill exists.
...
## Procedure
1. Analyze the request and identify the specific requirements for pagination filtering sorting.
2. Review existing patterns or codebase constraints before taking action.
3. Formulate a plan to execute the pagination filtering sorting operation safely.
...
## Checklist
- [ ] Understand the exact goal of the pagination filtering sorting request.
...
## Example Usage
*Scenario:* An agent is tasked with pagination filtering sorting.
