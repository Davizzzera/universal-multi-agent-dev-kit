...
type: skill
category: backend
description: Create consistent and safe error handling patterns.
when_to_use: When performing tasks related to error handling standard.
do_not_use_when: When the task falls outside this domain or requires a different skill entirely.
risk_level: medium
owner_agents: [backend-specialist, api-designer, security-reviewer]
...
## Purpose
Create consistent and safe error handling patterns.

## When to Use
Use this skill when tasked with executing a error handling standard operation to ensure consistency and correctness.

## When Not to Use
Do not use this skill if the task does not involve error handling standard, or if a more specific alternative skill exists.
...
## Procedure
1. Analyze the request and identify the specific requirements for error handling standard.
2. Review existing patterns or codebase constraints before taking action.
3. Formulate a plan to execute the error handling standard operation safely.
...
## Checklist
- [ ] Understand the exact goal of the error handling standard request.
...
## Example Usage
*Scenario:* An agent is tasked with error handling standard.
