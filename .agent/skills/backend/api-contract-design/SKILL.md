...
type: skill
category: backend
description: Design consistent request, response, error and endpoint contracts.
when_to_use: When performing tasks related to api contract design.
do_not_use_when: When the task falls outside this domain or requires a different skill entirely.
risk_level: medium
owner_agents: [api-designer, backend-specialist]
...
## Purpose
Design consistent request, response, error and endpoint contracts.

## When to Use
Use this skill when tasked with executing a api contract design operation to ensure consistency and correctness.

## When Not to Use
Do not use this skill if the task does not involve api contract design, or if a more specific alternative skill exists.
...
## Procedure
1. Analyze the request and identify the specific requirements for api contract design.
2. Review existing patterns or codebase constraints before taking action.
3. Formulate a plan to execute the api contract design operation safely.
...
## Checklist
- [ ] Understand the exact goal of the api contract design request.
...
## Example Usage
*Scenario:* An agent is tasked with api contract design.
