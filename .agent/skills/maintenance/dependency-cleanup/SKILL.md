...
description: Identify unused or risky dependencies for cleanup.
when_to_use: When performing tasks related to dependency cleanup.
do_not_use_when: When the task falls outside this domain or requires a different skill entirely.
risk_level: medium
owner_agents: [dependency-security-auditor, technical-lead]
...
## Purpose
Identify unused or risky dependencies for cleanup.

## When to Use
Use this skill when tasked with executing a dependency cleanup operation to ensure consistency and correctness.

## When Not to Use
Do not use this skill if the task does not involve dependency cleanup, or if a more specific alternative skill exists.
...
## Procedure
1. Analyze the request and identify the specific requirements for dependency cleanup.
2. Review existing patterns or codebase constraints before taking action.
3. Formulate a plan to execute the dependency cleanup operation safely.
...
## Checklist
- [ ] Understand the exact goal of the dependency cleanup request.
...
## Example Usage
*Scenario:* An agent is tasked with dependency cleanup.
