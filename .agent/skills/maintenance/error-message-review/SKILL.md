...
description: Improve user-facing and developer-facing error messages.
when_to_use: When performing tasks related to error message review.
do_not_use_when: When the task falls outside this domain or requires a different skill entirely.
risk_level: medium
owner_agents: [qa-tester, backend-specialist, documentation-writer]
...
## Purpose
Improve user-facing and developer-facing error messages.

## When to Use
Use this skill when tasked with executing a error message review operation to ensure consistency and correctness.

## When Not to Use
Do not use this skill if the task does not involve error message review, or if a more specific alternative skill exists.
...
## Procedure
1. Analyze the request and identify the specific requirements for error message review.
2. Review existing patterns or codebase constraints before taking action.
3. Formulate a plan to execute the error message review operation safely.
...
## Checklist
- [ ] Understand the exact goal of the error message review request.
...
## Example Usage
*Scenario:* An agent is tasked with error message review.
