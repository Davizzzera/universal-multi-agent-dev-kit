...
description: Create a structured code review checklist.
when_to_use: When performing tasks related to code review checklist.
do_not_use_when: When the task falls outside this domain or requires a different skill entirely.
risk_level: medium
owner_agents: [technical-lead, qa-tester]
...
## Purpose
Create a structured code review checklist.

## When to Use
Use this skill when tasked with executing a code review checklist operation to ensure consistency and correctness.

## When Not to Use
Do not use this skill if the task does not involve code review checklist, or if a more specific alternative skill exists.
...
## Procedure
1. Analyze the request and identify the specific requirements for code review checklist.
2. Review existing patterns or codebase constraints before taking action.
3. Formulate a plan to execute the code review checklist operation safely.
...
## Checklist
- [ ] Understand the exact goal of the code review checklist request.
...
## Example Usage
*Scenario:* An agent is tasked with code review checklist.
