...
description: Audit performance across UI, API, database or build flow.
when_to_use: When performing tasks related to performance audit.
...
## Purpose
Audit performance across UI, API, database or build flow.

## When to Use
Use this skill when tasked with executing a performance audit operation to ensure consistency and correctness.

## When Not to Use
Do not use this skill if the task does not involve performance audit, or if a more specific alternative skill exists.
...
## Procedure
1. Analyze the request and identify the specific requirements for performance audit.
2. Review existing patterns or codebase constraints before taking action.
3. Formulate a plan to execute the performance audit operation safely.
...
## Checklist
- [ ] Understand the exact goal of the performance audit request.
...
## Example Usage
*Scenario:* An agent is tasked with performance audit.
