...
description: Identify debt, fragility and maintainability risks.
when_to_use: When performing tasks related to technical debt detection.
do_not_use_when: When the task falls outside this domain or requires a different skill entirely.
risk_level: medium
owner_agents: [technical-lead, refactor-specialist]
...
## Purpose
Identify debt, fragility and maintainability risks.

## When to Use
Use this skill when tasked with executing a technical debt detection operation to ensure consistency and correctness.

## When Not to Use
Do not use this skill if the task does not involve technical debt detection, or if a more specific alternative skill exists.
...
## Procedure
1. Analyze the request and identify the specific requirements for technical debt detection.
2. Review existing patterns or codebase constraints before taking action.
3. Formulate a plan to execute the technical debt detection operation safely.
...
## Checklist
- [ ] Understand the exact goal of the technical debt detection request.
...
## Example Usage
*Scenario:* An agent is tasked with technical debt detection.
