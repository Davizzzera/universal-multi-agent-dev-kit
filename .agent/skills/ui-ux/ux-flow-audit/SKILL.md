...
type: skill
category: ui-ux
description: Review user journeys, friction points and interaction flow.
when_to_use: When performing tasks related to ux flow audit.
do_not_use_when: When the task falls outside this domain or requires a different skill entirely.
risk_level: medium
owner_agents: [ui-ux-designer, product-manager]
...
## Purpose
Review user journeys, friction points and interaction flow.

## When to Use
Use this skill when tasked with executing a ux flow audit operation to ensure consistency and correctness.

## When Not to Use
Do not use this skill if the task does not involve ux flow audit, or if a more specific alternative skill exists.
...
## Procedure
1. Analyze the request and identify the specific requirements for ux flow audit.
2. Review existing patterns or codebase constraints before taking action.
3. Formulate a plan to execute the ux flow audit operation safely.
...
## Checklist
- [ ] Understand the exact goal of the ux flow audit request.
...
## Example Usage
*Scenario:* An agent is tasked with ux flow audit.
