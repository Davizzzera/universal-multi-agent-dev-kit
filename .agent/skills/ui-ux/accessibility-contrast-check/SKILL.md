...
type: skill
category: ui-ux
description: Validate contrast, readability and basic accessibility requirements.
when_to_use: When performing tasks related to accessibility contrast check.
do_not_use_when: When the task falls outside this domain or requires a different skill entirely.
risk_level: medium
owner_agents: [accessibility-specialist, ui-ux-designer]
...
## Purpose
Validate contrast, readability and basic accessibility requirements.

## When to Use
Use this skill when tasked with executing a accessibility contrast check operation to ensure consistency and correctness.

## When Not to Use
Do not use this skill if the task does not involve accessibility contrast check, or if a more specific alternative skill exists.
...
## Procedure
1. Analyze the request and identify the specific requirements for accessibility contrast check.
2. Review existing patterns or codebase constraints before taking action.
3. Formulate a plan to execute the accessibility contrast check operation safely.
...
## Checklist
- [ ] Understand the exact goal of the accessibility contrast check request.
...
## Example Usage
*Scenario:* An agent is tasked with accessibility contrast check.
