...
name: coordinate
type: workflow
description: Coordinate multiple agents for complex tasks that require multi-agent analysis, execution and validation.
trigger: ["coordinate", "orchestrate", "multi-agent", "use multiple agents", "complex task"]
primary_agent: universal-orchestrator
required_agents: ["project-context-reader", "task-router", "scope-guardian", "conflict-controller", "final-reviewer"]
optional_agents: ["any specialist agent depending on task classification"]
required_skills: ["coordinator-mode", "agent-routing", "parallel-work-planning", "file-ownership-locking", "handoff-protocol", "task-breakdown", "implementation-plan", "change-impact-summary"]
...
# Workflow Purpose
Coordinate multiple agents for complex tasks that require multi-agent analysis, execution and validation.

## Important Behavior
- Parallelize discovery.
- Serialize conflicting edits.
- Parallelize verification.
- Use the conflict controller before any multi-file or multi-agent implementation.
...
- **Required Agents:** project-context-reader, task-router, scope-guardian, conflict-controller, final-reviewer
- **Optional Agents:** any specialist agent depending on task classification
...
# Execution Steps
1. **Trigger Reception**: The Orchestrator identifies the user intent mapping to the `coordinate` workflow.
...
# Example Run
*User Request:* "Please perform a coordinate operation."
