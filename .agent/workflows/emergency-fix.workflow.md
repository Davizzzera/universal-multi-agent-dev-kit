...
name: emergency-fix
type: workflow
description: Handle urgent fixes safely while preserving scope, validation and rollback discipline.
trigger: ["emergency fix", "hotfix", "urgent bug", "production issue", "critical issue"]
primary_agent: universal-orchestrator
required_agents: ["scope-guardian", "conflict-controller", "bug-hunter", "qa-tester", "final-reviewer"]
optional_agents: ["backend-specialist", "frontend-specialist", "devops-specialist", "security-reviewer", "database-specialist", "release-manager"]
required_skills: ["task-breakdown", "implementation-plan", "rollback-plan", "test-plan-create", "regression-test-checklist", "build-validation", "change-impact-summary", "security-release-checklist if applicable"]
...
# Workflow Purpose
Handle urgent fixes safely while preserving scope, validation and rollback discipline.

## Important Behavior
- Keep scope extremely narrow.
- Fix only the urgent issue.
- Require rollback plan.
- Document risks and validation evidence.
- Do not use emergency context as permission for broad rewrites.
...
- **Required Agents:** scope-guardian, conflict-controller, bug-hunter, qa-tester, final-reviewer
- **Optional Agents:** backend-specialist, frontend-specialist, devops-specialist, security-reviewer, database-specialist, release-manager
...
- security-release-checklist if applicable

# Execution Steps
1. **Trigger Reception**: The Orchestrator identifies the user intent mapping to the `emergency-fix` workflow.
...
# Example Run
*User Request:* "Please perform a emergency fix operation."
