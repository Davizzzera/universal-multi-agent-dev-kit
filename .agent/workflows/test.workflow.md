...
name: test
type: workflow
description: Create, improve or run tests for existing behavior, new features or critical flows.
trigger: ["test", "create tests", "improve tests", "validate flow", "test coverage"]
primary_agent: universal-orchestrator
required_agents: ["task-router", "qa-tester", "final-reviewer"]
optional_agents: ["unit-test-specialist", "e2e-test-specialist", "backend-specialist", "frontend-specialist", "visual-regression-specialist", "devops-specialist"]
required_skills: ["test-plan-create", "unit-test-create", "integration-test-create", "e2e-test-create", "build-validation", "regression-test-checklist", "acceptance-validation"]
...
# Workflow Purpose
Create, improve or run tests for existing behavior, new features or critical flows.

## Important Behavior
- Tests should match real requirements.
- Avoid brittle tests.
...
- **Required Agents:** task-router, qa-tester, final-reviewer
- **Optional Agents:** unit-test-specialist, e2e-test-specialist, backend-specialist, frontend-specialist, visual-regression-specialist, devops-specialist
...
# Execution Steps
1. **Trigger Reception**: The Orchestrator identifies the user intent mapping to the `test` workflow.
...
# Example Run
*User Request:* "Please perform a test operation."
